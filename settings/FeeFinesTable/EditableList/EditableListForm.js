...
function validate(values) {
	...
	if(values.items[0]) {
		if(!values.items[0].feefinetype) {
			errors.items[0].feefinetype = 'Please fill this to continue';
		}
	}
...
class EditableListForm extends React.Component {
  ...
  renderItems({ fields }) {
    return (
      <div>
        <Row between="xs">
				<Col xs>FEE/FINE TYPE*</Col>
				<Col xs>DEFAULT AMOUNT</Col>
				<Col xs>ALLOW MANUAL CREATION?</Col>
				<Col xs>TAX/VAT %</Col>
          <Col xs>
            <Row end="xs">
              <Col xs>
